...
def do_login():
    ...
    cookies = {"XSRF-TOKEN": csrf_header, "othelloarena_session": session}
    ...
    payload = {"_token": csrf_token, "email": E_MAIL, "password": PASSWORD}
    response = requests.post(
        LOGIN_URL, headers=headers, cookies=cookies, data=payload, allow_redirects=False
    )
    if response.status_code == requests.codes.found:
        logging.info(
            "[*] Successfully logged in! code: {}".format(response.status_code)
        )
    else:
...
def make_room(auth_info):
    ...
    tokens = get_cookie_and_token(
        ROOM_CREATE_URL, auth_info["headers"], auth_info["cookies"]
    )
    payload = {
        "_token": tokens["csrf-token"],
        "name": "MAGCHO_WEAK_BOT",
        "size": "8",
        "play_with_mine": "0",
        "alpha": "0",
        "is_history": "1",
    ...
    auth_info["cookies"]["othelloarena_session"] = response.cookies[
        "othelloarena_session"
    ]

    if response.status_code == requests.codes.ok:
        logging.info(
            "[*] Successfully created room! code: {}".format(response.status_code)
        )
    else:
        logging.info(
            "[!] You have already created room! code: {}".format(response.status_code)
        )
...
def destroy_room(auth_info):
    ...
    if response.status_code == requests.codes.found:
        logging.info(
            "[*] Successfully destroyed room! code: {}".format(response.status_code)
        )
        make_room(auth_info)
    else:
...
def do_logout(auth_info):
    ...
    csrf_token = get_cookie_and_token(
        HOME_URL, auth_info["headers"], auth_info["cookies"]
    )["csrf-token"]
    ...
    if response.status_code == requests.codes.ok:
        logging.info(
            "[*] Successfully logged out! code: {}".format(response.status_code)
        )
    else:
        logging.critical("[!] Unknown err...")
        sys.exit(-1)
